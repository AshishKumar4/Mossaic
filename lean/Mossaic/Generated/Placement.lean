...
Modeled file: shared/placement.ts (~78 LoC). Exposes only the pure
rendezvous-score primitives (`placeChunk`, `placeFile`,
`computePoolSize`); DO addressing happens directly via
...
What we DO state, as a structural property: the shard-name builder
called downstream of `placeChunk` is `vfsShardDOName`, whose injectivity
is proved in `Mossaic.Vfs.Tenant.shardName_inj_fixed_idx`. So the TS
sequence:
    sc → idx ← placeChunk(userId, fileKey, chunkIndex, poolSize)
