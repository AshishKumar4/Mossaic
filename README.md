...
- **Universal preview pipeline** — `vfs.readPreview()` returns rendered preview bytes (image/webp or image/svg+xml) for any file: images via Cloudflare Images, source code as syntax-highlighted SVG, audio as waveform sparkline, fallback icon-card for everything else; variants are content-addressed and refcount-shared across users; `Cache-Control: public, max-age=31536000, immutable` with weak-ETag revalidation; batched `vfs.openManifests()` for galleries (one round-trip per N paths). See [`docs/previews.md`](./docs/previews.md).
